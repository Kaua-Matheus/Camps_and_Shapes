...
func _resolve_target() -> void:
	...
	if nodes.is_empty():
		push_error("PlayerHUD: nenhum node no grupo 'Player' encontrado.")
		return
...
func _setup_skill_hud() -> void:
	...
	for i in range(3):
		...
		bg_rect.position = Vector2(x, start_y)
		bg_rect.size = Vector2(icon_size, icon_size)
		bg_rect.color = colors[i]
		add_child(bg_rect)
...
"""
	...
	add_child(vignette)
	target._freeze_shader = mat  # injeta referência no player
 
	# Label de status de frio
	var lbl := Label.new()
	...
	lbl.visible = false
	add_child(lbl)
	target._freeze_label = lbl  # injeta referência no player
 
 
# ── Skill Logic ───────────────────────────────────────────────────────
func _handle_skills(delta: float) -> void:
	# Decrementa cooldowns (o player.gd não faz mais isso)
